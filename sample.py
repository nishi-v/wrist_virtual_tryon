...
# Initialize session state attributes if not already done
...
if not st.session_state.uploaded_image:
    uploaded_file = st.file_uploader("Upload an image of the wrist", type=["jpg", "jpeg", "png"])
    ...
    if uploaded_file is not None:
        ...
        # Display uploaded wrist image
        st.session_state.wrist_image = cv2.imdecode(np.frombuffer(uploaded_file.getbuffer(), np.uint8), cv2.IMREAD_COLOR)
        st.session_state.wrist_image = cv2.cvtColor(st.session_state.wrist_image, cv2.COLOR_BGR2RGB)
        st.image(st.session_state.wrist_image, caption='Uploaded Wrist Image', use_column_width=True)

        # Set the flag to indicate the image has been uploaded
...
if st.session_state.uploaded_image:
    if not st.session_state.bracelet_selected:
        ...
        for name, image_path in bracelets.items():
            bracelet_image = Image.open(image_path).convert("RGBA")
            st.image(bracelet_image, caption=name, width=200)
            
            if st.button(f"Try On {name}"):
                st.session_state.bracelet_selected = True
                st.session_state.selected_bracelet = name
                st.session_state.object = bracelet_image
                break  # Exit the loop after a bracelet is selected

    else:
        # Overlay Selected Bracelet
        object = st.session_state.object
        img = st.session_state.wrist_image
        ...
        files = [
            ('image', ('temp_image.jpg', open('temp_image.jpg', 'rb'), 'image/jpeg'))
        ]
        ...
        center_pixel = (int(center_coords[0] * img_width), int(center_coords[1] * img_height))
        ...
        # Calculate the length of the bracelet (distance between left and right pixel)
        ...
        # Provide option to try another bracelet
        if st.button("Change Bracelet"):
            st.session_state.bracelet_selected = False
            st.session_state.object = None
